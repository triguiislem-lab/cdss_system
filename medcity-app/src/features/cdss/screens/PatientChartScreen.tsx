...
import { getPatientFullName, patients, prescriptions } from "@/lib/mock-data";
...
export default function PatientChart({ basePath }: { basePath: "/admin" | "/doctor" }) {
  const params = useParams<{ id?: string; patientId?: string }>();
  const workspaceBase = basePath === "/admin" ? "/admin/cdss" : "/doctor";
      ...
      <div className="p-4 lg:p-8">
        <div className="rounded-xl border border-dashed border-border bg-card p-10 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            The patient record you requested is not available in the demo dataset.
          </p>
          <Link
  ...
  const patientCases = prescriptions.filter((entry) => entry.patientId === patient.id);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Unified patient chart for the MedCity Connect workspace.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link
            href={`${workspaceBase}/prescription/new?patientId=${encodeURIComponent(patient.id)}`}
          ...
          <Link
            href={basePath === "/admin" ? "/admin/cdss/prescription/review" : "/doctor/prescriptions"}
        ...
        <div className="lg:col-span-8 xl:col-span-9 space-y-4">
          <section className="rounded-xl border border-border bg-card shadow-card overflow-hidden">
            <div className="px-5 py-4 border-b border-border">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Snapshot of active issues that affect prescribing decisions.
              </p>
            </div>
            <div className="grid gap-4 md:grid-cols-3 p-5">
              <Metric
                icon={AlertTriangle}
                label="Risk flags"
                value={String(patient.flags.length)}
                note={patient.flags.length > 0 ? patient.flags.join(", ") : "No active flags"}
              />
              <Metric
                icon={Pill}
                label="Active medications"
                value={String(patient.currentMedications.length)}
                note={
              ...
              <Metric
                icon={Activity}
                label="Comorbidities"
                value={String(patient.comorbidities.length)}
                note={patient.comorbidities.join(", ") || "No chronic conditions listed"}
              />
            </div>
          </section>
          ...
          <section className="rounded-xl border border-border bg-card shadow-card overflow-hidden">
            <div className="px-5 py-4 border-b border-border">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Current and recent recommendation history for this patient.
              </p>
            </div>
            {patientCases.length === 0 ? (
              <div className="p-8 text-center text-sm text-muted-foreground">
                No prescription cases available for this patient yet.
              </div>
            ) : (
                    ...
                    <article key={entry.id} className="p-5">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          <span className={`inline-flex rounded-full border px-2.5 py-0.5 text-xs font-semibold ${risk.cls}`}>
                            {risk.label}
                          </span>
                        </div>
                      </div>
